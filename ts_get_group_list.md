...
ナビゲーションの「開発」タブの「セキュリティ設定」にある「信頼できる認証」を有効化することでトークンが取得できます。このトークンを貼り付けてください。くれぐれもトークンの取り扱いにはご注意ください。シークレットキーが記載されている場合は「信頼できる認証」（Trusted Auth）で認証が行われます。

...
#    - ファイルをORG,Groupごとに分割して生成します
#    - APIで一度に取得するレコード数を100にします# python ts_get_group_list.py -j "credential_trial.json"
...
def tsapi_search_user_groups(settings, session, recordsize, include_users, include_subusers):
    base_url = settings['base_url']
    post_data = {
    "record_offset": 0,
    "record_size": recordsize,
    "include_users": include_users, # ユーザーを含むかどうか True/False
    "include_sub_groups": include_subusers # サブグループを含むかどうか True/False
    }
    api_headers = {
    'X-Requested-By': 'ThoughtSpot',
    'Accept': 'application/json',
...
def main(jsonfile, all_org_processing, divide_files, recordsize):
    try:
        ...
        # ========= Get User Groups and belonging users ========= 
        print("Processing User Groups list...")
        groups_json = tsapi_search_user_groups(setting, requests_session, recordsize, True, False)
        df_groups = pd.json_normalize(groups_json)
        df_groups['org_name'] = df_groups['orgs'].apply(_extract_org_name) # Org名を分解

        # ========= 所属ユーザーを取得
        # ===== group名,userid、user名のみに変換
        df_groups_subset = df_groups[['name', 'users','org_name']]
        df_groups_subset = df_groups_subset.rename(columns={'name': 'group_name'})
        # NaNを空文字に置換してから、文字列であるものだけパースする
        df_groups_subset['users'] = df_groups_subset['users'].fillna('[]').apply(lambda x: ast.literal_eval(x) if isinstance(x, str) and x.startswith('[') else x)
        # リストを縦方向に展開
        df_expanded = df_groups_subset.explode('users')
        # 辞書の中身を列に分ける
        df_groupuser_master = pd.concat([
            df_expanded.drop('users', axis=1), 
            df_expanded['users'].apply(pd.Series)
        ], axis=1)
        # 確実に必要な列だけを再選択する
        valid_columns = ['org_name','group_name', 'id', 'name'] 
        df_groupuser_master = df_groupuser_master[[col for col in valid_columns if col in df_groupuser_master.columns]]
        df_groupuser_master = df_groupuser_master.rename(columns={'id': 'user_id','name': 'user_name'})
        ...
        if divide_files:
            ...
            for org_name, group in df_groups.groupby('org_name'):
                ...
                export_df = group.drop(columns=['org_name']) # 保存用のorg_nameを削除
                export_df.to_csv(filename, index=False, encoding='utf-8-sig')
                print(f"Saved: {filename}")

            # User List
            for (org_name, group_name),group_df in df_groupuser_master.groupby(['org_name','group_name']):
                filename = f"ts_usergroupusers_{setting['settingname']}_{org_name}_{group_name}.csv"
                
                export_df = group_df.drop(columns=['org_name','group_name']) # 保存用のorg_name,group_nameを削除
                export_df.to_csv(filename, index=False, encoding='utf-8-sig')
                print(f"Saved: {filename}")
            ...
            # User List
            filename = f"ts_usergroupusers_{setting['settingname']}.csv"
            df_groupuser_master.to_csv(filename, index=False, encoding='utf-8-sig')
            print(f"Saved: {filename}")
